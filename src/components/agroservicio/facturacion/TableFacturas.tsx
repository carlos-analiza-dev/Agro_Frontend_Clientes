...
import { ResponseFacturasAgroInterface } from "@/api/agroservicio/facturacion/interface/response-facturas-agro.interface";
import { AgroFactura } from "../../../api/agroservicio/facturacion/interface/response-facturas-agro.interface";
import { ResponseExistenciaInterface } from "@/api/agroservicio/facturacion/interface/verificar-existencia.interface";
import { useFacturaDownload } from "@/helpers/funciones/agroservicio/facturacion/download_factura_agro";
import { VerificarExistencia } from "@/api/agroservicio/facturacion/accions/verificar-existencia";
import { ProcesarFactura } from "@/api/agroservicio/facturacion/accions/procesar-factura";
import { CancelarFactura } from "@/api/agroservicio/facturacion/accions/cancelar-factura";
import { AutorizarCancelacionFactura } from "@/api/agroservicio/facturacion/accions/autorizar-factura";

interface Props {
  facturas: ResponseFacturasAgroInterface | undefined;
...
const TableFacturas = ({
  ...
  const [facturaAProcesar, setFacturaAProcesar] = useState<AgroFactura | null>(
    null,
  );
  const [facturaACancelar, setFacturaACancelar] = useState<AgroFactura | null>(
    null,
  );
  const [facturaAAutorizar, setFacturaAAutorizar] =
    useState<AgroFactura | null>(null);
  ...
  const handlePreviewFactura = async (factura: AgroFactura) => {
    try {
      const objectUrl = await verFactura(
        factura.id,
        propietarioId,
  ...
  const handleAbrirAutorizacionCancelacion = (factura: AgroFactura) => {
  ...
  const handleAbrirConfirmacionProcesar = async (factura: AgroFactura) => {
  ...
  const handleAbrirConfirmacionCancelar = (factura: AgroFactura) => {
  ...
  const mostrarBotonAutorizar = (factura: AgroFactura) => {
    return (
      isPropietario &&
          ...
          {facturas?.data?.map((factura) => (
            <TableRow key={factura.id}>
              ...
              <TableCell className="text-center">
                {formatCurrency(factura.sub_total, moneda)}
              </TableCell>
              <TableCell className="text-center font-semibold">
                {formatCurrency(factura.total, moneda)}
              </TableCell>
                ...
                <div className="flex gap-2 justify-center">
                  ...
                  {factura.estado === "Procesada" && (
                    <>
                      ...
                      {(factura.autorizada_cancelacion || isPropietario) && (
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() =>
            ...
            <div className="text-sm text-muted-foreground space-y-3">
              ...
              <div className="bg-purple-50 p-3 rounded-md text-sm border border-purple-200">
                ...
                <p>
                  <strong>Total:</strong>{" "}
                  {facturaAAutorizar &&
                    formatCurrency(facturaAAutorizar.total, moneda)}
                </p>
              </div>
            ...
            <div className="text-sm text-muted-foreground space-y-3">
              ...
              <div className="bg-green-50 p-3 rounded-md text-sm border border-green-200">
                ...
                <p>
                  <strong>Total:</strong>{" "}
                  {facturaAProcesar &&
                    formatCurrency(facturaAProcesar.total, moneda)}
                </p>
              </div>
              <p className="text-xs text-gray-500">
            ...
            <div className="text-sm text-muted-foreground space-y-3">
              ...
              <div className="bg-red-50 p-3 rounded-md text-sm border border-red-200">
                ...
                <p>
                  <strong>Total:</strong>{" "}
                  {facturaACancelar &&
                    formatCurrency(facturaACancelar.total, moneda)}
                </p>
              </div>
              <p className="text-xs text-gray-500">
