...
import { InsumoAgroCompra } from "@/api/agroservicio/compras_insumos/interfaces/insumos_compra.interface";
import {
  CrearCompraAgroInsumo,
  CrearCompraAgroInsumoEmpleado,
} from "@/api/agroservicio/compras_insumos/accions/crear-compra-insumo";
...
import { ObtenerEscalasProveedorAndInsumo } from "@/api/agroservicio/escalas-insumos/accions/escalas-proveedor-insumo";
import { tiposPagos } from "@/helpers/data/compras/tiposPagos";
import ResumenCompra from "../compras/ResumenCompra";
import DetailsCompra from "../compras/DetailsCompra";
...
const FormCompraInsumos = ({
  ...
  const crearCompraMutation = useMutation({
    mutationFn: isPropietario
      ...
      : CrearCompraAgroInsumoEmpleado,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["compras-agro-insumos"] });
  ...
  return (
    <>
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div
          className={`grid grid-cols-1 gap-4 p-4 bg-gray-50 rounded-lg ${isPropietario ? "md:grid-cols-4" : "md:grid-cols-3"}`}
        >
          {isPropietario && (
            <div className="space-y-1">
              <Label className="font-bold">Sucursal destino *</Label>
              <Select
              ...
              {!selectedSucursalId && isPropietario && (
                <p className="text-sm text-red-500">Seleccione una sucursal</p>
              )}
            </div>
          )}
              ...
              <div key={field.id} className="p-4 border rounded-lg space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <Select
                      value={insumosWatch?.[index]?.cantidad?.toString() || ""}
                      onValueChange={(value) => {
