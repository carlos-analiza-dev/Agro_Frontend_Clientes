...
import { Escala } from "@/api/agroservicio/escalas-insumos/interfaces/response-escala-insumo.interface";
import useGetEscalasInsumos from "@/hooks/agroservicios/escalas-insumos/useGetEscalasInsumos";
import { CrearEscalaAgroInsumo } from "@/api/agroservicio/escalas-insumos/accions/crear-escala-insumo";
import { CrearEscalaAgroInsumoInterface } from "@/api/agroservicio/escalas-insumos/interfaces/crear-escala-insumo.interface";
import { ActualizarEscalaAgroInsumo } from "@/api/agroservicio/escalas-insumos/accions/actualizar-escala-insumo";
...
const TableEscalaInsumo = ({
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<CrearEscalaAgroInsumoInterface>();

  const [escalas, setEscalas] = useState<EscalaLocal[]>([]);

  const createMutation = useMutation({
    mutationFn: CrearEscalaAgroInsumo,
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: {
      ...
      return ActualizarEscalaAgroInsumo(data.id, updateData);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({
        queryKey: ["escalas-insumo", limit, offset, selectedInsumo?.id],
  ...
  const onSubmit = (data: CrearEscalaAgroInsumoInterface) => {
    if (!selectedInsumo) {
          ...
          {escalas.length === 0 ? (
            <TableRow>
              <TableCell colSpan={7} className="text-center py-4">
                <MessageError
                  titulo="Error al cargar la escala"
                  descripcion="Se produjo un error al momento de cargar las escalas del insumo seleccionado"
                />
              </TableCell>
            </TableRow>
          ) : (
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              unidades
              <br />
              <strong>Costo:</strong> {moneda} {selectedEscala?.costo}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
